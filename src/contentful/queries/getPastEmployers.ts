...
const createPastEmployerResponseObj = (
	...
	companyEntryFields: InstitutionResponse,
	...
	description: documentToHtmlString(pastEmployerEntryFields.description),
});

const getCompanyFields = (pastEmployerEntryFields: PastEmployerFields) =>
	(
		pastEmployerEntryFields.company as {
			fields: InstitutionResponse;
		}
	).fields;

const getLocationFields = async (pastEmployerEntryFields: PastEmployerFields) =>
	(
...
export const getPastEmployers = async () =>
	await Promise.all(
		(await getPastEmployerEntries()).map(
			async ({ fields: pastEmployerEntryFields }) =>
